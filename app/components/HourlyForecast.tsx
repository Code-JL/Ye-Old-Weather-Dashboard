...
import { convertTemperature, convertWindSpeed } from '@/app/utils/unitConversions';
...
export default function HourlyForecast({ data }: HourlyForecastProps) {
  ...
  const hourlyData = data.time.map((time, index): HourData | null => {
  ...
  const morningHours = hourlyData.filter(hour => hour.hour < 12);
  const afternoonHours = hourlyData.filter(hour => hour.hour >= 12);
